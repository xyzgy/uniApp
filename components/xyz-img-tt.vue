<template>
	<view>
		<!-- <view class="shadeTop" style="position: relative;" :style="{ height: shadowTH + 'rpx' }"> -->
			<cover-view class="" style="position: absolute;z-index:9999999;width:100%;height: 100%;" :style="{ height: shadowTH + 'rpx' }">
				<cover-view class="topList" style="display: flex;justify-content: space-around;align-items: center;">
					<cover-view v-for="(item, index) in tabList" :key="index" :class="tadIdx == index ? 'tabActive' : ''" @click="tabClick(index)">{{ item.title }}</cover-view>
				</cover-view>
			</cover-view>
		<!-- </view> -->
		<cover-view style="position: absolute;z-index:9999999;width:100%;height:200upx;bottom: 40upx;">
			<cover-view class=" " style="position: relative;height: 100%;">
				<cover-view style="position: absolute;width:100%;bottom: 42upx;">
					<cover-view style="display: flex;justify-content: space-around;align-items: center;">
						<cover-view
							class="text_center"
						...
						<cover-view style="position: relative;"><button class="margin_auto btn_style" @click="fUpload">确定</button></cover-view>
					</cover-view>
				</cover-view>
			</cover-view>
		</cover-view>
		<!-- <image :src="imgSrc.imgSrc" :style="[imgStyle]" @click="fSelect" class="my-avatar"></image> -->
		<canvas :style="{ top: styleTop, height: cvsStyleHeight }" canvas-id="avatar-canvas" class="my-canvas" disable-scroll="false" id="avatar-canvas"></canvas>
		<canvas
			...
			id="oper-canvas"
		></canvas>
		<!-- <canvas :style="{ height: cvsStyleHeight, top: prvTop }" @touchstart="fHideImg" canvas-id="prv-canvas" class="prv-canvas" disable-scroll="false" id="prv-canvas"></canvas> -->
		<!-- <view :style="{ display: styleDisplay }" class="oper-wrapper">
			<view class="oper">
				<view class="btn-wrapper" v-if="showOper">
					<view :style="{ width: btnWidth }" @click="fSelect" hover-class="hover"><text>重选</text></view>
					<view :style="{ width: btnWidth }" @click="fClose" hover-class="hover"><text>关闭</text></view>
					<view :style="{ width: btnWidth, display: btnDsp }" @click="fRotate" hover-class="hover"><text>旋转</text></view>
					<view :style="{ width: btnWidth }" @click="fPreview" hover-class="hover"><text>预览</text></view>
					<view :style="{ width: btnWidth }" @click="fUpload" hover-class="hover"><text>上传</text></view>
				</view>
				<view class="clr-wrapper" v-else="">
					<slider
						...
						value="0"
					></slider>
					<view :style="{ width: btnWidth }" @click="fPrvUpload" hover-class="hover"><text>上传</text></view>
				</view>
			</view>
		</view> -->
	</view>
...
<script>
const tabHeight = 0; //底部选项卡高度 默认50
export default {
	name: 'yq-avatar',
	data() {
		return {
			...
			tadIdx: 0,
			shadowTH: 140,
			selWidth: this.rectWidth,
			selHeight: this.rectHeight,
	...
	props: {
		...
		avatarStyle: {
			type: String,
			default: 'width: 750upx; height: 360upx; border-radius:0'
		},
		rectWidth: {
			type: String,
			default: '750upx'
		...
		isCenter: {
			type: Boolean,
			default: false
		}, //改装增加变量 true选框及图片垂直居中，false指定top值
		lW: {
			type: Number,
			default: 2
		}, //线宽
	...
	methods: {
		tabClick(idx) {
			...
			if (idx === 0) {
				this.selStyle.height = uni.upx2px(360) + 'px';
			} else if (idx === 1) {
				this.selStyle.height = uni.upx2px(750) + 'px';
			} else if (idx === 2) {
				this.selStyle.height = uni.upx2px(850) + 'px';
			}
						...
						success: r => {
							...
							if (!this.hasSel) {
								...
								if (this.selWidth && this.selHeight) {
									...
									// style.top = uni.upx2px(this.shadowTH) + 'px';
		...
		fDrawInit(ini = false) {
			...
			// 增加类似蒙版效果
			ctxCanvasOper.setFillStyle('#202020');
			...
			ctxCanvasOper.setLineWidth(this.lW);
			// 左上

			ctxCanvasOper.moveTo(left + 20 + this.lW, top);
			ctxCanvasOper.lineTo(left + this.lW, top);
			ctxCanvasOper.lineTo(left + this.lW, top + 20);
			// 右上
			ctxCanvasOper.moveTo(left + width - 20 - this.lW, top);
			ctxCanvasOper.lineTo(left + width - this.lW, top);
			ctxCanvasOper.lineTo(left + width - this.lW, top + 20);
			// 左下
			ctxCanvasOper.moveTo(left + 20 + this.lW, top + height);
			ctxCanvasOper.lineTo(left + this.lW, top + height);
			ctxCanvasOper.lineTo(left + this.lW, top + height - 20);
			// 右下
			ctxCanvasOper.moveTo(left + width - 20 - this.lW, top + height);
			ctxCanvasOper.lineTo(left + width - this.lW, top + height);
			ctxCanvasOper.lineTo(left + width - this.lW, top + height - 20);
			...
			ctxCanvasOper.draw(false, () => {
				if (ini) {
					...
					this.styleTop = '0';
					// #endif
					// ctxCanvas.setFillStyle(this.canvasColor);
					this.fDrawImage();
				}
			});
			...
			uni.canvasToTempFilePath(
				{
					...
					success: r => {
						...
						ctxCanvasPrv.setFillStyle(this.canvasColor);
		...
		async fColorChange(e) {
			...
			this.ctxCanvasPrv.setFillStyle(this.canvasColor);
		...
		async initPath(path, typ) {
			if (typ === 'true') {
				this.path = this.imgPath =
					this.$const.imgPath +
					(await this.$store.dispatch('proxyImgPath', {
						path: path,
						w: 750,
						q: 80
					}));
			...
			let imgInfo = await this.getImageInfo( this.imgPath);
			if (imgInfo) {
				this.imgPath = this.path = imgInfo.path;
	...
	mounted() {
		// console.log(e)
		// if (e.isEdit) {
		// 	this.initPath(e.path, e.isEdit);
		// } else {
		// 	uni.showToast({
		// 		title:'加载异常，请返回重试',
		// 		icon:'none'
		// 	})
			this.initPath('http://www.mangowed.com/uploads/allimg/150327/1-15032H24K0149.jpg', false);
		// }
	}
};
</script>
